...
public class ExcelTools : EditorWindow
{
	...
	/// <summary>
	/// 输出格式 {"JSON","CSV","XML","LUA"}
	/// </summary>
	private static string[] formatOption=new string[]{"JSON","CSV"};
	...
	/// 第一行默认为策划用的中文语义
	/// </summary>
	private static bool isDesc=true;
	...
	private void DrawOptions()
	{
		...
		isDesc=GUILayout.Toggle(isDesc,"第一行默认为策划用的中文语义");
	...
	private static void Convert()
	{
		foreach(string assetsPath in excelList)
		{
			...
			if(indexOfFormat==0){
				...
				excel.ConvertToJson(output,encoding, isDesc);
	...
	private static void LoadExcel()
	{
		...
		{
			var dirPath = Application.dataPath + "/Excel";
			...
			foreach (string path in Directory.GetFiles(dirPath))
			{
...

				{
					...
					excelList.Add(path.Substring(path.IndexOf("Assets")));
				}
			}
